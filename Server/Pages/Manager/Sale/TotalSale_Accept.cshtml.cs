...
public class TotalSale_AcceptModel : BasePageModel
{
    private readonly ITotalSaleApplication _application;

    public readonly DatabaseContext _context;

    public TotalSale_AcceptModel(ITotalSaleApplication application,
    ...
    {
        ViewModel = await _context.TotalSales.OrderBy(x => x.FactorNumber).Where(x => x.Accepted == true).ToListAsync();
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        ViewModel = await _context.TotalSales.Where(x => x.FactorNumber == factor && x.Accepted == true).ToListAsync();
