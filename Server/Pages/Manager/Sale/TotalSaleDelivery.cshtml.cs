...
public class TotalSaleDeliveryModel : BasePageModel
{
    private readonly ITotalSaleApplication _application;

    public readonly DatabaseContext _context;

    public TotalSaleDeliveryModel(ITotalSaleApplication application,
    ...
    {
        ViewModel = await _context.TotalSales.OrderBy(x => x.FactorNumber).Where(x => x.Delivery == true).ToListAsync();
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        ViewModel = await _context.TotalSales.Where(x => x.FactorNumber == factor && x.Delivery == true).ToListAsync();
